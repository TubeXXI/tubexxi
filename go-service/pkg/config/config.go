...
func Load() (*Config, error) {
	...
	config := &Config{
		...
		JWT: JWTConfig{
			...
			JwtExpiration:        getAsTime("JWT_EXPIRATION", 15*time.Minute),
			JwtRefreshExpiration: getAsTime("JWT_REFRESH_EXPIRATION", 24*time.Hour),
		},
		Centrifugo: CentrifugoConfig{
		...
		MinIO: MinIOConfig{
			...
			MinioBucketName:      getEnv("MINIO_BUCKET_NAME", "indoxxi"),
