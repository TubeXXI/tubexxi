...
class ScraperService(scraper_pb2_grpc.ScraperServiceServicer):
    ...
    def GetSeriesDetail(self, request, context):
        if not request.slug:
             context.set_details("Slug is required")
             context.set_code(grpc.StatusCode.INVALID_ARGUMENT)
             return scraper_pb2.SeriesDetailResponse()
        
        url = request.slug
        if not url.startswith('http'):
            url = f"{self.SERIES_BASE_URL}/{request.slug}"
            
        try:
            html_content = fetch_html(url)
            scraper = SeriesDetailScraper(html_content, base_url=self.SERIES_BASE_URL)
            detail = scraper.scrape(original_url=url)
            
            if not detail:
                context.set_details("Failed to scrape series details")
                context.set_code(grpc.StatusCode.NOT_FOUND)
                return scraper_pb2.SeriesDetailResponse()

            # Map Movie
            m = detail.movie_detail.movie
            proto_movie = scraper_pb2.Movie(
                id=m.id,
                title=m.title,
                original_title=m.original_title,
            ...
            def map_people(people):
                return [scraper_pb2.MoviePerson(name=p.name, page_url=p.page_url) for p in (people or [])]
                
            def map_players(players):
                return [scraper_pb2.PlayerUrl(url=p.url, type=p.type) for p in (players or [])]

            md = detail.movie_detail
            proto_movie_detail = scraper_pb2.MovieDetail(
                movie=proto_movie,
                votes=md.votes,
                release_date=md.release_date,
                updated_at=md.updated_at,
                player_urls=map_players(md.player_urls),
                trailer_url=md.trailer_url,
                directors=map_people(md.directors),
                movie_stars=map_people(md.movie_stars),
                countries=[scraper_pb2.CountryMovie(name=c.name, page_url=c.page_url) for c in (md.countries or [])],
                genres=[scraper_pb2.Genre(name=g.name, page_url=g.page_url) for g in (md.genres or [])],
                similar_movies=[
                    scraper_pb2.Movie(
                        id=sm.id, title=sm.title, original_title=sm.original_title, thumbnail=sm.thumbnail,
                        synopsis=sm.synopsis, rating=sm.rating, duration=sm.duration, year=sm.year,
                        date_published=sm.date_published, label_quality=sm.label_quality, genre=sm.genre,
                        original_page_url=sm.original_page_url
                    ) for sm in (md.similar_movies or [])
                ]
            )
            
            # Map Season List
            proto_season_list = []
            if detail.season_list:
                for s in detail.season_list:
                    ep_list = []
                    if s.episode_list:
                        for ep in s.episode_list:
                            ep_list.append(scraper_pb2.EpisodeList(
                                episode_number=ep.episode_number,
                                episode_url=ep.episode_url,
                                player_urls=map_players(ep.player_urls),
                                trailer_url=ep.trailer_url
                            ))
                    
                    proto_season_list.append(scraper_pb2.SeasonList(
                        current_season=s.current_season,
                        total_season=s.total_season,
                        episode_list=ep_list
                    ))

            series_detail = scraper_pb2.SeriesDetail(
                movie_detail=proto_movie_detail,
                season_name=detail.season_name,
                status=detail.status,
                season_list=proto_season_list
            )
            
            return scraper_pb2.SeriesDetailResponse(detail=series_detail)
    ...
    def GetSeriesEpisode(self, request, context):
        if not request.url:
             context.set_details("URL is required")
             context.set_code(grpc.StatusCode.INVALID_ARGUMENT)
             return scraper_pb2.SeriesEpisodeResponse()

        try:
            html_content = fetch_html(request.url)
            scraper = SeriesEpisodeScraper(html_content, base_url=self.SERIES_BASE_URL)
            result = scraper.scrape(url=request.url)
            
            if not result:
                context.set_details("Failed to scrape episode")
                context.set_code(grpc.StatusCode.NOT_FOUND)
                return scraper_pb2.SeriesEpisodeResponse()
            
            return scraper_pb2.SeriesEpisodeResponse(
                episode_number=result.episode_number,
                player_urls=[scraper_pb2.PlayerUrl(url=p.url, type=p.type) for p in result.player_urls],
