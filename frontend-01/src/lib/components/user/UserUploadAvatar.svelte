...
<script lang="ts">
	...
	async function uploadToServer() {
		...
		try {
			...
			const response = await fetch('/user/account/avatar', {
				method: 'POST',
				body: formData
			});
			...
			if (!response.ok || !data.success) {
				toast.error(data.message || i18n.update_avatar_failed());
				files = [];
				return;
			}
			if (data.success) {
				toast.success(data.message || i18n.update_profile_success());
				await invalidateAll();
				files = [];
			}
		} catch (error) {
			toast.error(error instanceof Error ? error.message : i18n.update_avatar_failed());
		} finally {
			isUploading = false;
			await invalidateAll();
			open = false;
...
<Dialog.Root bind:open onOpenChange={(val) => val && invalidateAll()}>
	...
	<Dialog.Content>
		<Dialog.Header>
			<Dialog.Title>{i18n.upload_profile_avatar()}</Dialog.Title>
			<Dialog.Description>{i18n.upload_profile_avatar_description()}</Dialog.Description>
		</Dialog.Header>
			...
			{#if isUploading}
				<Empty.Root class="w-full">
					<Empty.Header>
						...
						<Empty.Title>{i18n.please_wait()}</Empty.Title>
						<Empty.Description>{i18n.processing_request()}</Empty.Description>
					</Empty.Header>
				</Empty.Root>
			{:else}
				<FileDropZone.Root
		...
		<Dialog.Footer>
			<Button type="button" onclick={uploadToServer}>{i18n.upload_profile_avatar()}</Button>
			<Dialog.Close>
				<Button variant="destructive" size="default">{i18n.close()}</Button>
			</Dialog.Close>
		</Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>
