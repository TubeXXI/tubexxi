...
export class UserServiceImpl extends BaseService implements UserService {
	...
	async UpdateAvatar(file: File): Promise<string | Error> {
		try {
			...
			const response = await this.api.multipartAuthRequest<{ avatar_url?: string }>('POST', '/user/protected/avatar', formData);
			if (!response.success) {
	...
	async UpdatePassword(data: UpdatePasswordSchema): Promise<string | Error> {
		try {
			const response = await this.api.authRequest<{ message?: string }>('PUT', '/user/protected/password', data);
			if (!response.success) {
