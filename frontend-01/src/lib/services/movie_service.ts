...
export class MovieServiceImpl extends BaseService implements MovieService {
	...
	async GetMoviesByGenre(slug: string, page: number = 1): Promise<PaginatedResult<Movie>> {
		try {
			const response = await this.api.publicRequest<Movie[]>('GET', `/movies/genre/${slug}&page=${page}`);
			if (!response.success) {
	...
	async GetMoviesByCountry(country: string, page: number = 1): Promise<PaginatedResult<Movie>> {
		try {
			const response = await this.api.publicRequest<Movie[]>('GET', `/movies/country/${country}&page=${page}`);
			if (!response.success) {
	...
	async GetMoviesByYear(year: number, page: number = 1): Promise<PaginatedResult<Movie>> {
		try {
			const response = await this.api.publicRequest<Movie[]>('GET', `/movies/year/${year}&page=${page}`);
			if (!response.success) {
	...
	async SearchMovies(query: string, page: number = 1): Promise<PaginatedResult<Movie>> {
		try {
			const response = await this.api.publicRequest<Movie[]>('GET', `/movies/search?s=${query}&page=${page}`);
			if (!response.success) {
	...
	async GetMoviesByFeature(type: string, page: number = 1): Promise<PaginatedResult<Movie>> {
		try {
			const response = await this.api.publicRequest<Movie[]>('GET', `/movies/featured/${type}&page=${page}`);
			if (!response.success) {
	...
	async GetSpecialPage(path: string, page: number = 1): Promise<PaginatedResult<Movie>> {
		try {
			const response = await this.api.publicRequest<Movie[]>('GET', `/movies/special/${path}&page=${page}`);
			if (!response.success) {
