...
export class SeriesServiceImpl extends BaseService implements SeriesService {
	...
	async GetSeriesByGenre(slug: string, page: number = 1): Promise<PaginatedResult<Movie>> {
		try {
			const response = await this.api.publicRequest<Movie[]>('GET', `/series/genre/${slug}&page=${page}`);
			if (!response.success) {
	...
	async GetSeriesByCountry(country: string, page: number = 1): Promise<PaginatedResult<Movie>> {
		try {
			const response = await this.api.publicRequest<Movie[]>('GET', `/series/country/${country}&page=${page}`);
			if (!response.success) {
	...
	async GetSeriesByYear(year: number, page: number = 1): Promise<PaginatedResult<Movie>> {
		try {
			const response = await this.api.publicRequest<Movie[]>('GET', `/series/year/${year}&page=${page}`);
			if (!response.success) {
	...
	async SearchSeries(query: string, page: number = 1): Promise<PaginatedResult<Movie>> {
		try {
			const response = await this.api.publicRequest<Movie[]>('GET', `/series/search?s=${query}&page=${page}`);
			if (!response.success) {
	...
	async GetSeriesByFeature(type: string, page: number = 1): Promise<PaginatedResult<Movie>> {
		try {
			const response = await this.api.publicRequest<Movie[]>('GET', `/series/featured/${type}&page=${page}`);
			if (!response.success) {
	...
	async GetSeriesSpecialPage(path: string, page: number = 1): Promise<PaginatedResult<Movie>> {
		try {
			const response = await this.api.publicRequest<Movie[]>('GET', `/series/special/${path}&page=${page}`);
			if (!response.success) {
	...
	async GetSeriesEpisode(url: string): Promise<SeriesEpisode | null> {
		try {
			const response = await this.api.publicRequest<SeriesEpisode>('GET', `/series/episode?url=${url}`);
			if (!response.success) {
