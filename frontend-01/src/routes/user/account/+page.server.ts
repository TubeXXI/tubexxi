...
export const load = async ({ locals, parent }) => {
	...
	const title = (await deps.languageHelper.singleTranslate('Account - Profile', lang)) as SingleResponse;
...
export const actions = {
	default: async ({ request, locals }) => {
		...
		if (response instanceof Error) {
			return fail(500, {
				form,
				message: response.message || i18n.unknown_error_message()
			});
		}
		...
		return {
			form,
			message: i18n.update_profile_success()
		};
	}
}